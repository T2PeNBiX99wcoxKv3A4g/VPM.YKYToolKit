...
namespace io.github.ykysnk.ykyToolkit.Discord
{
    ...
    {
        [PublicAPI] public readonly Result Result;
    ...
    public class Discord : IDisposable
    {
        ...
        {
            ApplicationManagerInstance ??= new(
        ...
        {
            RelationshipManagerInstance ??= new(
        ...
        {
            AchievementManagerInstance ??= new(
    ...
    public partial class ActivityManager
    {
        ...
        {
            var wrapped = GCHandle.Alloc(callback);
            Methods.SendRequestReply(_methodsPtr, userId, reply, GCHandle.ToIntPtr(wrapped),
    ...
    public partial class LobbyManager
    {
        ...
        {
            var wrapped = GCHandle.Alloc(callback);
            Methods.UpdateLobby(_methodsPtr, lobbyId, transaction.MethodsPtr, GCHandle.ToIntPtr(wrapped),
        ...
        {
            var wrapped = GCHandle.Alloc(callback);
            Methods.ConnectLobbyWithActivitySecret(_methodsPtr, activitySecret, GCHandle.ToIntPtr(wrapped),
        ...
        {
            var wrapped = GCHandle.Alloc(callback);
            Methods.UpdateMember(_methodsPtr, lobbyId, userId, transaction.MethodsPtr, GCHandle.ToIntPtr(wrapped),
        ...
        {
            var wrapped = GCHandle.Alloc(callback);
            Methods.SendLobbyMessage(_methodsPtr, lobbyId, data, data.Length, GCHandle.ToIntPtr(wrapped),
        ...
        private static void OnLobbyMessageImpl(IntPtr ptr, long lobbyId, long userId, IntPtr dataPtr, int dataLen)
        {
            ...
            d.LobbyManagerInstance.OnLobbyMessage(lobbyId, userId, data);
        }
    ...
    public class NetworkManager
    {
        ...
        private static void OnMessageImpl(IntPtr ptr, ulong peerId, byte channelId, IntPtr dataPtr, int dataLen)
        {
            var h = GCHandle.FromIntPtr(ptr);
            var d = (Discord)h.Target;
            if (d.NetworkManagerInstance?.OnMessage == null) return;
            var data = new byte[dataLen];
            Marshal.Copy(dataPtr, data, 0, dataLen);
    ...
    public class OverlayManager
    {
        ...
        {
            var wrapped = GCHandle.Alloc(callback);
            Methods.SetImeCompositionRangeCallback(_methodsPtr, GCHandle.ToIntPtr(wrapped),
        ...
        {
            var wrapped = GCHandle.Alloc(callback);
            Methods.SetImeSelectionBoundsCallback(_methodsPtr, GCHandle.ToIntPtr(wrapped),
                SetImeSelectionBoundsCallbackCallbackImpl);
        }

        public bool IsPointInsideClickZone(int x, int y) => Methods.IsPointInsideClickZone(_methodsPtr, x, y);
        ...
        internal struct FFIMethods
        {
            ...
            internal delegate Result
                InitDrawingDxgiMethod(IntPtr methodsPtr, IntPtr swapchain, bool useMessageForwarding);
    ...
    public partial class StorageManager
    {
        ...
        {
            var wrapped = GCHandle.Alloc(callback);
            Methods.ReadAsyncPartial(_methodsPtr, name, offset, length, GCHandle.ToIntPtr(wrapped),
    ...
    public class AchievementManager
    {
        ...
        {
            var wrapped = GCHandle.Alloc(callback);
            Methods.SetUserAchievement(_methodsPtr, achievementId, percentComplete, GCHandle.ToIntPtr(wrapped),
